...
function MotionNavigationMenu({
  ...
  const openValue = React.useCallback(
    ...
    [cancelClose, getItemValues, setActiveValue, updateViewportPosition],
  );
  ...
  const closeMenu = React.useCallback(() => {
    ...
    setActiveValue("");
    updateViewportPosition();
  }, [cancelClose, setActiveValue, updateViewportPosition]);
...
function MotionNavigationMenuList({
  ...
  return (
    <Highlight
      ...
      hover
      className={cn(
        "bg-accent rounded-none pointer-events-none",
        highlightClassName,
      )}
      style={{ zIndex: -1 }}
...
function MotionNavigationMenuViewport({
  ...
  const isMega = context?.layout === "mega";
  const activeContent =
  ...
  React.useLayoutEffect(() => {
    ...
    const updateSize = () => {
      ...
      const nextSize = {
        width: isMega ? window.innerWidth : rect.width,
        height: rect.height,
      };
  ...
  if (isMega) {
    return (
      <motion.div
        className={cn(
          "fixed inset-x-0 top-14 z-50 isolate md:top-16",
          activeContent ? "pointer-events-auto" : "pointer-events-none",
        )}
        initial={false}
        onPointerEnter={() => context?.cancelClose()}
        onPointerLeave={(event) => {
          const related = event.relatedTarget;
          const nav = (event.currentTarget as HTMLElement).closest(
            '[data-slot="navigation-menu"]',
          );
      ...
      >
        <motion.div
          ...
          transition={context?.spring}
          className={cn(
            "relative w-screen overflow-hidden border-0 bg-white shadow-none",
            className,
          )}
        >
          <AnimatePresence
        ...
        <div
          ...
          aria-hidden="true"
          data-slot="navigation-menu-measure"
          className="pointer-events-none invisible absolute top-0 left-0 w-screen"
        >
          {activeContent && (
            <div className={cn(activeContent.className)}>
              <MotionNavigationMenuContentInner
                highlightClassName={activeContent.highlightClassName}
                innerClassName={activeContent.innerClassName}
              >
  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={context?.spring}
        className={cn(
          "bg-background text-popover-foreground relative mt-1.5 overflow-hidden rounded-none border shadow backdrop-blur-md",
          className,
        )}
      >
        <AnimatePresence
