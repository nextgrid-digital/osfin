...
export interface CoverflowCarouselProps {
  ...
  /** Card centered on first paint. */
  initialIndex?: number;
  /** Fires when an arrow, key, or drag release finishes on a card. */
    ...
    (target: number) => {
      ...
      const step = () => {
        ...
        if (Math.abs(remaining) < 0.0004) {
          ...
          paint();
          rafRef.current = null;
          onSettleRef.current?.(indexAt(target));
          return;
        }
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-none bg-[#E4E4E4] shadow-xl will-change-transform",
                  cardClassName,
                )}
